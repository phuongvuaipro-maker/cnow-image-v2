...
export default function App() {
  ...
  const handleGeneratePrompt = async () => {
    ...
    try {
      ...
      if (!apiKeyToUse) throw new Error("Vui lòng nhập Gemini API Key của bạn ở cột bên trái.");
      // @ts-ignore
      ...
      if (msg.includes("429") || msg.includes("quota")) {
        setError("Bạn đã tạo quá nhanh hoặc hết lượt dùng. Vui lòng đợi 1 phút rồi thử lại nhé (Lỗi 429).");
      } else if (msg.includes("API key not valid") || msg.includes("400")) {
        setError("API Key không hợp lệ. Vui lòng kiểm tra lại (Lỗi 400).");
      } else {
        setError(msg || "Failed to generate prompt");
      }
    } finally {
  ...
  const handleGenerateImages = async () => {
    ...
    try {
      // @ts-ignore
      const apiKey = userApiKey || (usePremiumModel ? (process.env.API_KEY || process.env.GEMINI_API_KEY) : process.env.GEMINI_API_KEY);
      if (!apiKey) throw new Error("Vui lòng nhập Gemini API Key của bạn ở cột bên trái.");
      // @ts-ignore
      ...
      const promises = Array.from({ length: count }).map(() => generateImage());
      ...
      <aside className="w-full md:w-96 bg-zinc-900 border-r border-zinc-800 flex flex-col h-screen overflow-y-auto">
        ...
        <div className="p-6 space-y-8">
          ...
          <section className="space-y-4">
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">API Configuration</h2>
            <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800/50 space-y-3">
              <label className="text-xs text-zinc-500">Your Gemini API Key</label>
              <input 
                type="password" 
                value={userApiKey}
              ...
              <p className="text-xs text-zinc-500">
                Lấy key miễn phí tại <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-emerald-400 hover:underline">Google AI Studio</a>. Key được lưu an toàn trên trình duyệt của bạn.
              </p>
            </div>
          </section>
